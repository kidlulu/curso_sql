...
# 2	PROCEDIMENTOS PARA INSTALAÇÃO DE FERRAMENTAS PARA CONSULTAS EM BANCO DE DADOS
...
O *DBeaver* é um software livre utilizado para acesso, administração e consultas a diversos SGBD. Ele pode ser instalado em diversos sistemas operacionais. No caso do Windows, ele pode ser instalado e configurado sem a necessidade de perfil administrativo, como mostra os passos descritos abaixo.
...
•	***CONVERT***(tipo, coluna ou expressão) ***AS*** apelido: Esta é uma função tem o poder de converter diversos tipos de dados existentes em uma tabela. Por exemplo transformar uma coluna com informações do tipo '*integer*' para o '*float*', usa-se a programação ***SELECT CONVERT***(***FLOAT***, Nome da coluna no formato ***INTEGER***);
